...
def EA_plt_land(f, domain, steps, a=15, b=-80):
    ...
    CS = ay.contour(X, Y, Z, levels, cmap=cm.viridis_r)
...
def EA_plt_pop(f, domain, steps, genera_res, run_s, gen_s, a=15, b=-80):
    ...
    surf = ax.plot_surface(X, Y, Z, cmap='cividis_r', alpha=0.25)
    ax.set_aspect('auto')
    ax.autoscale_view()
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ...
    ay.contour(X, Y, Z, levels, cmap='cividis_r', linewidths=0.25)
    ay.set_aspect('auto')
    ay.set_xlabel('x')
    ay.set_ylabel('y')
...
def EA_plt_gen(f, domain, steps, genera_res, run_s, gen_s, a=15, b=-80):
    ...
    ax.plot_surface(X, Y, Z, cmap='cividis_r', alpha=0.25)
    ax.set_aspect('auto')
    ax.autoscale_view()
    ax.set_xlabel('x')
    ax.set_ylabel('y')
