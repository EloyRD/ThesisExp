...
# %%
# %%time
fig, axs = plt.subplots(figsize=(12, 12), nrows=3, ncols=3, sharex=True, sharey=True, constrained_layout=True)
pop_plot_list = [-1,0,1]
...
for i, row in enumerate(axs):
    for j, cell in enumerate(row):
        Z = pred_1_2(pop_plot_list[j], Y, cx_pb_plot, mut_pb_plot, X, k_par_plot, k_sur_plot_list[i])
        ax = axs[i, j]
        ...
        if j == 0:
            cell.set_ylabel("at k_sur="+str(k_sur_title[i])+"\n"+"b_ratio", fontsize="medium")
